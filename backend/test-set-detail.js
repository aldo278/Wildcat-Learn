async function testSetDetail() {
  try {
    ...
    console.log('\n2. Getting user sets...');
    const setsResponse = await fetch('http://localhost:3001/api/sets/', {
      headers: {
    ...
    const setDetailResponse = await fetch(`http://localhost:3001/api/sets/${firstSet.id}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
    ...
    const cardsResponse = await fetch(`http://localhost:3001/api/cards/set/${firstSet.id}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
